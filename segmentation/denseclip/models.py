...
class CLIPResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # --- Start CHANGE ---
        outs = [] # Initialize list to store outputs
        x = x.type(self.conv1.weight.dtype)
        x = stem(x)

        x = self.layer1(x)
        outs.append(x) # Append layer 1 output

        x = self.layer2(x)
        outs.append(x) # Append layer 2 output

        x = self.layer3(x)
        outs.append(x) # Append layer 3 output

        x = self.layer4(x)
        outs.append(x) # Append layer 4 output

        # Return a tuple of the collected outputs
        return tuple(outs)
        # --- End CHANGE ---
